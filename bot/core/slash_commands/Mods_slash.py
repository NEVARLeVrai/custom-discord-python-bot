...
class Mods_slash(commands.Cog):
    ...
    async def clear(self, interaction: discord.Interaction, amount: int):
        ...
        if not interaction.user.guild_permissions.manage_messages:
            await interaction.response.send_message(t('help_no_permission'), ephemeral=True)
            return
        ...
        if amount > max_amount:
            await interaction.followup.send(t('mods_spam_max_error', max=max_amount), ephemeral=False)
            amount = max_amount
        
        if amount < 1:
            await interaction.followup.send(t('mods_spam_min_error'), ephemeral=True)
            return
        ...
        try:
            ...
            try:
                ...
                await interaction.followup.send(t('mods_clear_success', count=len(deleted)), ephemeral=False, wait=False)
            except (discord.NotFound, discord.HTTPException) as e:
                # Webhook expired or HTTP error - send normal message to channel
                try:
                    if interaction.channel:
                        embed = discord.Embed(
                            title=t('mods_clear_title'),
                            description=t('mods_clear_success', count=len(deleted)),
                    ...
                    print(t('log_err_msg_send', error=channel_error))
            except Exception as e:
                # Other error - log but continue
                print(t('log_err_msg_send', error=e))
                
        except discord.Forbidden:
            error_embed = discord.Embed(
                title=t('mods_error_title'),
                description=t('mods_clear_perm_error'),
            ...
            error_embed = discord.Embed(
                title=t('mods_error_title'),
                description=t('mods_clear_error', error=str(e)),
                color=discord.Color.red()
            )
            error_embed.set_footer(text=get_current_version(self.client))
            try:
                ...
                print(t('log_err_msg_send', error=send_error))
    ...
    async def kick(self, interaction: discord.Interaction, member: discord.Member, reason: str = None):
        """Kick a member"""
        if reason is None:
            reason = t('mods_kick_reason_default')
        # Check permissions
        ...
        if member.id == interaction.user.id:
            await interaction.response.send_message(t('mods_no_self_kick'), ephemeral=True)
            return
        
        # Check role hierarchy
        if interaction.user.top_role <= member.top_role and interaction.user.id != interaction.guild.owner_id:
            await interaction.response.send_message(t('mods_kick_hierarchy_error'), ephemeral=True)
            return
        ...
        try:
            # Send DM to user
            try:
                kick_dm = discord.Embed(title=t('mods_dm_kick_title'), description=t('mods_dm_kick_desc', server=interaction.guild.name), color=discord.Color.yellow())
                kick_dm.add_field(name=t('mods_moderator_field'), value=f"{interaction.user.name} ({interaction.user.mention})", inline=False)
                kick_dm.add_field(name=t('mods_reason_field'), value=reason, inline=False)
            ...
            # Confirmation
            conf_embed = discord.Embed(title=t('mods_success_title'), description="", color=discord.Color.yellow())
            conf_embed.set_author(name=t('help_requested_by', user=interaction.user.name), icon_url=interaction.user.avatar)
            conf_embed.add_field(name=t('mods_kicked_field'), value=t('mods_kicked_desc', member=member.mention, moderator=interaction.user.mention), inline=False)
            conf_embed.add_field(name=t('mods_reason_field'), value=reason, inline=False)
            ...
            error_embed = discord.Embed(title=t('mods_error_title'), description=t('mods_kick_perm_error'), color=discord.Color.red())
            await interaction.followup.send(embed=error_embed, ephemeral=True)
        except Exception as e:
            error_embed = discord.Embed(title=t('mods_error_title'), description=t('mods_error_occurred', error=str(e)), color=discord.Color.red())
            await interaction.followup.send(embed=error_embed, ephemeral=True)
    ...
    async def warn(self, interaction: discord.Interaction, member: discord.Member, reason: str, count: int = 1):
        ...
        if not isinstance(interaction.channel, discord.TextChannel):
            await interaction.response.send_message(t('mods_no_mp_error'), ephemeral=True)
            return

        # Checks
        if interaction.user.id == member.id:
            await interaction.response.send_message(t('mods_no_self_warn'), ephemeral=True)
            return
        
        if member.bot:
            await interaction.response.send_message(t('mods_no_bot_warn'), ephemeral=True)
            return
        ...
        if count < 1:
            count = 1
        if count > 10:
            await interaction.response.send_message(t('mods_warn_max_count_error'), ephemeral=True)
            return
        ...
        # Confirmation embed
        conf_embed = discord.Embed(title=t('mods_success_title'), description="", color=discord.Color.orange())
        conf_embed.set_author(name=t('help_requested_by', user=interaction.user.name), icon_url=interaction.user.avatar)
        
        if count > 1:
            conf_embed.add_field(name=t('mods_dm_warn_title'), value=t('mods_warn_added', member=member.mention, count=count, moderator=interaction.user.mention), inline=False)
        else:
            conf_embed.add_field(name=t('mods_dm_warn_title'), value=t('mods_warn_added_single', member=member.mention, moderator=interaction.user.mention), inline=False)
        
        conf_embed.add_field(name=t('mods_reason_field'), value=reason, inline=False)
        conf_embed.add_field(name=t('mods_warn_total_field'), value=f"{total_warn_count}", inline=False)
        conf_embed.set_footer(text=get_current_version(self.client))
        ...
        if not member.bot:
            try:
                warn_dm = discord.Embed(title=t('mods_dm_warn_title'), description=t('mods_dm_warn_desc', server=interaction.guild.name), color=discord.Color.orange())
                if count > 1:
                    warn_dm.add_field(name=t('mods_dm_warn_title'), value=t('mods_warn_added', member=member.mention, count=count, moderator=interaction.user.mention), inline=False)
                else:
                    warn_dm.add_field(name=t('mods_moderator_field'), value=f"{interaction.user.name} ({interaction.user.mention})", inline=False)
    ...
    async def resetwarn(self, interaction: discord.Interaction, member: discord.Member):
        ...
        if self.moderation_service.reset_warns(interaction.guild.id, member.id):
            conf_embed = discord.Embed(title=t('mods_success_title'), description=t('mods_reset_warn_desc', member=member.mention), color=discord.Color.green())
            conf_embed.set_author(name=t('help_requested_by', user=interaction.user.name), icon_url=interaction.user.avatar)
            conf_embed.set_footer(text=get_current_version(self.client))
            await interaction.followup.send(embed=conf_embed)
        else:
            await interaction.followup.send(t('mods_reset_warn_no_warns', member=member.mention), ephemeral=True)
    ...
    async def warnboard(self, interaction: discord.Interaction):
        ...
        if guild_id not in warns or not warns[guild_id]:
            embed = discord.Embed(title=t('mods_lb_title'), description=t('mods_lb_empty'), color=discord.Color.orange())
        ...
        embed = discord.Embed(title=t('mods_lb_title'), description=t('mods_lb_desc', server=interaction.guild.name), color=discord.Color.orange())
    ...
    async def ban(self, interaction: discord.Interaction, user: str, reason: str):
        ...
        try:
            ...
            try:
                ...
                await interaction.followup.send(t('mods_ban_format_error'), ephemeral=True)
                return
            
            # Ban
            try:
                await interaction.guild.ban(discord.Object(id=target_id), reason=reason)
                embed = discord.Embed(title=t('mods_success_title'), description=t('mods_ban_id_desc', id=target_id, moderator=interaction.user.mention), color=discord.Color.red())
                embed.add_field(name=t('mods_reason_field'), value=reason, inline=False)
                embed.set_footer(text=get_current_version(self.client))
                await interaction.followup.send(embed=embed)
            except discord.Forbidden:
                await interaction.followup.send(t('mods_ban_perm_error'), ephemeral=True)
            except discord.NotFound:
                await interaction.followup.send(t('mods_mp_user_not_found'), ephemeral=True)
        except Exception as e:
            await interaction.followup.send(t('mods_error_occurred', error=str(e)), ephemeral=True)
    ...
    async def unban(self, interaction: discord.Interaction, user_id: str):
        ...
        try:
            ...
            conf_embed = discord.Embed(title=t('mods_success_title'), description="", color=discord.Color.green())
            conf_embed.set_author(name=t('help_requested_by', user=interaction.user.name), icon_url=interaction.user.avatar)
            conf_embed.add_field(name=t('mods_unban_title'), value=t('mods_unban_desc', id=user_id, moderator=interaction.user.mention), inline=False)
            conf_embed.set_footer(text=get_current_version(self.client))
            await interaction.followup.send(embed=conf_embed, ephemeral=False)
        except Exception as e:
            await interaction.followup.send(t('mods_unban_error', error=str(e)), ephemeral=True)
    ...
    async def banword(self, interaction: discord.Interaction, word: str):
        ...
        if self.moderation_service.add_banned_word(interaction.guild.id, word):
            embed = discord.Embed(title=t('mods_success_title'), description=t('mods_bw_added', word=word), color=discord.Color.green())
            embed.set_author(name=t('help_requested_by', user=interaction.user.name), icon_url=interaction.user.avatar)
            embed.set_footer(text=get_current_version(self.client))
            await interaction.followup.send(embed=embed)
        else:
            await interaction.followup.send(t('mods_bw_already_banned', word=word), ephemeral=True)
    ...
    async def unbanword(self, interaction: discord.Interaction, word: str):
        ...
        if self.moderation_service.remove_banned_word(interaction.guild.id, word):
            embed = discord.Embed(title=t('mods_success_title'), description=t('mods_bw_removed', word=word), color=discord.Color.green())
            embed.set_author(name=t('help_requested_by', user=interaction.user.name), icon_url=interaction.user.avatar)
            embed.set_footer(text=get_current_version(self.client))
            await interaction.followup.send(embed=embed)
        else:
            await interaction.followup.send(t('mods_bw_not_found', word=word), ephemeral=True)
    ...
    async def listbannedwords(self, interaction: discord.Interaction):
        ...
        if guild_id not in banned_words or not banned_words[guild_id]:
            embed = discord.Embed(title=t('mods_bw_list_title'), description=t('mods_bw_list_empty'), color=discord.Color.orange())
            await interaction.followup.send(embed=embed)
            return
        ...
        if len(words_list) > 20:
            words_text += t('mods_bw_list_more', count=len(words_list)-20)
            
        embed = discord.Embed(title=t('mods_bw_list_title'), description=words_text, color=discord.Color.blue())
        embed.set_author(name=t('help_requested_by', user=interaction.user.name), icon_url=interaction.user.avatar)
        embed.set_footer(text=get_current_version(self.client))
        await interaction.followup.send(embed=embed)
    ...
    async def spam(self, interaction: discord.Interaction, amount: int, channel: discord.TextChannel, message: str):
        ...
        embed = discord.Embed(title=t('mods_spam_sent_title'), description=t('mods_spam_sent_desc_channel', amount=amount, channel=channel.mention), color=discord.Color.green())
        embed.set_author(name=t('help_requested_by', user=interaction.user.name), icon_url=interaction.user.avatar)
        embed.set_footer(text=get_current_version(self.client))
    ...
    async def cleanraidsimple(self, interaction: discord.Interaction, name: str):
        ...
        if found:
            embed4 = discord.Embed(title=t('mods_raid_clean_title'), description=t('mods_raid_clean_deleting', channel=channeldel.name), color=discord.Color.yellow())
            embed4.set_author(name=t('help_requested_by', user=interaction.user.name), icon_url=interaction.user.avatar)
            ...
            embed3 = discord.Embed(title=t('mods_raid_clean_title'), description=t('mods_raid_clean_success', channel=channeldel.name), color=discord.Color.green())
            embed3.set_author(name=t('help_requested_by', user=interaction.user.name), icon_url=interaction.user.avatar)
            embed3.set_footer(text=get_current_version(self.client))
            await interaction.followup.send(embed=embed3, ephemeral=False)
            
        else:
            embed5 = discord.Embed(title=t('mods_raid_clean_title'), description=t('mods_raid_clean_not_found', name=name), color=discord.Color.red())
            embed5.set_author(name=t('help_requested_by', user=interaction.user.name), icon_url=interaction.user.avatar)
    ...
    async def cleanraidmultiple(self, interaction: discord.Interaction, raid_date: str, raid_time: str):
        ...
        try:
            ...
            embed6 = discord.Embed(title=t('mods_raid_clean_time_title'), description=t('mods_raid_clean_time_success', time=raid_datetime), color=discord.Color.green())
            embed6.set_author(name=t('help_requested_by', user=interaction.user.name), icon_url=interaction.user.avatar)
            embed6.set_footer(text=get_current_version(self.client))
            await interaction.followup.send(embed=embed6, ephemeral=False)
        except Exception as e:
            embed = discord.Embed(title=t('mods_error_title'), description=t('mods_raid_clean_time_error', error=str(e)), color=discord.Color.red())
            embed.set_author(name=t('help_requested_by', user=interaction.user.name), icon_url=interaction.user.avatar)
            embed.set_footer(text=get_current_version(self.client))
    ...
    async def giverole(self, interaction: discord.Interaction, member: discord.Member, role: discord.Role):
        ...
        try:
            await member.add_roles(role, reason=t('mods_giverole_reason', user=interaction.user))
            conf_embed = discord.Embed(title=t('mods_success_title'), description="", color=discord.Color.random())
            conf_embed.set_author(name=t('help_requested_by', user=interaction.user.name), icon_url=interaction.user.avatar)
            conf_embed.add_field(name=f"@{role.name}", value=t('mods_giverole_desc', user=member.mention), inline=False)
            conf_embed.set_footer(text=get_current_version(self.client))
            await interaction.followup.send(embed=conf_embed, ephemeral=False)
            
        except discord.Forbidden:
            conf_embed1 = discord.Embed(title=t('mods_error_title'), description="", color=discord.Color.red())
            conf_embed1.set_author(name=t('help_requested_by', user=interaction.user.name), icon_url=interaction.user.avatar)
            conf_embed1.add_field(name=t('mods_role_perm_error'), value=" ", inline=False)
            ...
            error_embed = discord.Embed(title=t('mods_error_title'), description=t('mods_unexpected_error', error=str(e)), color=discord.Color.red())
            await interaction.followup.send(embed=error_embed, ephemeral=True)
    ...
    async def removerole(self, interaction: discord.Interaction, member: discord.Member, role: discord.Role):
        ...
        try:
            await member.remove_roles(role, reason=t('mods_removerole_reason', user=interaction.user))
            conf_embed = discord.Embed(title=t('mods_success_title'), description="", color=discord.Color.random())
            conf_embed.set_author(name=t('help_requested_by', user=interaction.user.name), icon_url=interaction.user.avatar)
            conf_embed.add_field(name=f"@{role.name}", value=t('mods_removerole_desc', user=member.mention), inline=False)
            conf_embed.set_footer(text=get_current_version(self.client))
            await interaction.followup.send(embed=conf_embed, ephemeral=False)
    ...
    async def mp(self, interaction: discord.Interaction, user: discord.User, message: str):
        ...
        try:
            # Send message in DM
            await user.send(f"{t('mods_mp_user_dm_title', user=interaction.user.name, mention=interaction.user.mention)}\n\n{message}")
            
            # Save conversation so replies are forwarded
            self.moderation_service.set_mp_conversation(user.id, interaction.user.id)
            
            # Confirmation
            embed = discord.Embed(title=t('mods_mp_sent_title'), description=t('mods_mp_sent_desc', user=user.mention), color=discord.Color.green())
            embed.set_author(name=t('help_requested_by', user=interaction.user.name), icon_url=interaction.user.avatar)
            embed.add_field(name=t('mods_mp_field_message'), value=message[:500] + ("..." if len(message) > 500 else ""), inline=False)
            embed.set_footer(text=get_current_version(self.client))
            await interaction.followup.send(embed=embed, ephemeral=False)
            
        except discord.Forbidden:
            embed = discord.Embed(title=t('mods_error_title'), description=t('mods_mp_error_forbidden', user=user.mention), color=discord.Color.red())
            embed.set_author(name=t('help_requested_by', user=interaction.user.name), icon_url=interaction.user.avatar)
            embed.set_footer(text=get_current_version(self.client))
            await interaction.followup.send(embed=embed, ephemeral=True)
        except Exception as e:
            embed = discord.Embed(title=t('mods_error_title'), description=t('mods_unexpected_error', error=str(e)), color=discord.Color.red())
            embed.set_author(name=t('help_requested_by', user=interaction.user.name), icon_url=interaction.user.avatar)
            embed.set_footer(text=get_current_version(self.client))
            await interaction.followup.send(embed=embed, ephemeral=True)
