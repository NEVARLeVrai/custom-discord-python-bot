...
class MusicControlView(discord.ui.View):
    ...
    async def previous_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if previous:
            ...
            await interaction.followup.send(t('music_btn_previous_playing', title=previous['title']), ephemeral=True)
        else:
            await interaction.followup.send(t('music_btn_previous_none'), ephemeral=True)

    @discord.ui.button(emoji="⏪", label=t('music_btn_rewind_label'), style=discord.ButtonStyle.secondary, custom_id="music_rewind")
    async def rewind_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if new_seek is not None:
            await interaction.response.send_message(t('music_btn_seek_rewind', time=15), ephemeral=True)
        else:
            await interaction.response.send_message(t('music_seek_error'), ephemeral=True)
    ...
    async def pause_resume_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        if self.cog.audio_service.is_paused(interaction.guild):
            if self.cog.audio_service.resume(interaction.guild):
                await interaction.response.send_message(t('music_btn_resumed'), ephemeral=True)
            else:
                await interaction.response.send_message(t('music_btn_resume_none'), ephemeral=True)
        elif self.cog.audio_service.is_playing(interaction.guild):
            if self.cog.audio_service.pause(interaction.guild):
                await interaction.response.send_message(t('music_btn_paused'), ephemeral=True)
            else:
                await interaction.response.send_message(t('music_btn_pause_none'), ephemeral=True)
        else:
            await interaction.response.send_message(t('music_btn_nothing_playing'), ephemeral=True)

    @discord.ui.button(emoji="⏩", label=t('music_btn_forward_label'), style=discord.ButtonStyle.secondary, custom_id="music_forward")
    async def forward_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        new_seek = await self.cog.audio_service.seek_relative(interaction.guild, 15)
        if new_seek is not None:
            await interaction.response.send_message(t('music_btn_seek_forward', time=15), ephemeral=True)
        else:
            await interaction.response.send_message(t('music_seek_error'), ephemeral=True)
    ...
    async def skip_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        if self.cog.audio_service.is_playing(interaction.guild) or self.cog.audio_service.is_paused(interaction.guild):
            ...
            if voice:
                voice.stop()
            desc = t('music_btn_skipped_next', title=next_track['title']) if next_track else t('music_btn_skipped_empty')
            await interaction.response.send_message(desc, ephemeral=True)
        else:
            await interaction.response.send_message(t('music_btn_nothing_playing'), ephemeral=True)

    @discord.ui.button(emoji="🔉", label=t('music_btn_vol_down_label'), style=discord.ButtonStyle.secondary, custom_id="music_vol_down")
    async def volume_down_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.send_message(t('music_btn_vol_set', emoji="🔉", vol=new_volume), ephemeral=True)

    @discord.ui.button(emoji="🔊", label=t('music_btn_vol_up_label'), style=discord.ButtonStyle.secondary, custom_id="music_vol_up")
    ...
    @discord.ui.button(emoji="📋", label=t('music_btn_queue_label'), style=discord.ButtonStyle.secondary, custom_id="music_queue")
    async def queue_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        queue = self.cog.audio_service.get_queue(self.guild_id)
        if not queue:
            await interaction.response.send_message(t('music_btn_queue_empty'), ephemeral=True)
        else:
            queue_list = "\n".join([f"**{i+1}.** {v['title']}" for i, v in enumerate(queue[:10])])
            if len(queue) > 10:
                queue_list += f"\n\n{t('music_btn_queue_more', count=len(queue) - 10)}"
            await interaction.response.send_message(t('music_btn_queue_title', list=queue_list), ephemeral=True)
    ...
    async def loop_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.followup.send(t('music_btn_loop_enabled') if is_loop else t('music_btn_loop_disabled'), ephemeral=True)

    @discord.ui.button(emoji="⏹️", label=t('music_btn_stop_label'), style=discord.ButtonStyle.danger, custom_id="music_stop")
    async def stop_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.send_message(t('music_btn_stopped'), ephemeral=True)
...
class YoutubeSearchSelectView(discord.ui.View):
    def __init__(self, videos, cog):
        ...
        self.cog = cog
        options = [
            discord.SelectOption(label=t('yt_search_video_label', num=i+1), description=video['title'][:100], value=video['url'])
...
class YoutubeSearchSelect(discord.ui.Select):
    def __init__(self, options, videos, cog):
        super().__init__(placeholder=t('yt_search_select_placeholder'), min_values=1, max_values=1, options=options)
        self.videos = videos
        self.cog = cog
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if self.cog.audio_service.is_playing(interaction.guild) or self.cog.audio_service.is_paused(interaction.guild):
                ...
                embed.add_field(name=t('yt_progress'), value=self.cog.audio_service.create_progress_bar(interaction.guild.id), inline=False)
                embed.set_author(name=t('help_requested_by', user=interaction.user.name), icon_url=interaction.user.avatar)
                embed.set_footer(text=get_current_version(self.cog.client))
...
class Video_slash(commands.Cog):
    ...
    def check_queue(self, interaction: discord.Interaction):
        async def inner_check_queue():
            ...
            if queue:
                ...
                embed = discord.Embed(title=t('yt_play_title'), description=t('yt_playing_desc', title=next_video["title"]), color=discord.Color.green())
                embed.add_field(name=t('yt_progress'), value=self.audio_service.create_progress_bar(guild_id), inline=False)
                embed.set_author(name=t('help_requested_by', user=interaction.user.name), icon_url=interaction.user.avatar)
    ...
    async def _update_progress_loop(self, guild_id, message, embed):
        ...
        try:
            while guild_id in self.progress_tasks:
                ...
                embed.set_field_at(0, name=t('yt_progress'), value=new_progress, inline=False)
    ...
    async def mplay(self, interaction: discord.Interaction, url: str):
        ...
        try:
            ...
            if not (self.audio_service.is_playing(interaction.guild) or self.audio_service.is_paused(interaction.guild)):
                ...
                embed.add_field(name=t('yt_progress'), value=self.audio_service.create_progress_bar(interaction.guild.id), inline=False)
    ...
    async def msearch(self, interaction: discord.Interaction, query: str):
        ...
        try:
            ...
            await interaction.followup.send(
                content=t('yt_search_results_title'),
    ...
    async def mskip(self, interaction: discord.Interaction):
        if self.audio_service.is_playing(interaction.guild) or self.audio_service.is_paused(interaction.guild):
            ...
            queue = self.audio_service.get_queue(interaction.guild.id)
            next_track = queue[0] if queue else None
            desc = t('yt_skip_next', title=next_track['title']) if next_track else t('yt_skip_none')
            
            embed = discord.Embed(title=t('yt_skip_title'), description=desc, color=discord.Color.orange())
            embed.set_author(name=t('help_requested_by', user=interaction.user.name), icon_url=interaction.user.avatar)
            embed.set_footer(text=get_current_version(self.client))
    ...
    @app_commands.command(name="mleave", description="Disconnect the bot from voice channel")
    ...
    async def maddqueue(self, interaction: discord.Interaction, url: str):
        ...
        try:
            ...
            embed = discord.Embed(title=t('music_addqueue_title'), description=t('music_addqueue_desc', title=title, position=len(queue)), color=discord.Color.blue())
            embed.set_author(name=t('help_requested_by', user=interaction.user.name), icon_url=interaction.user.avatar)
            embed.set_footer(text=get_current_version(self.client))
            await interaction.followup.send(embed=embed)
    ...
    async def mremovequeue(self, interaction: discord.Interaction, position: int):
        ...
        if position < 1 or position > len(queue):
            embed = discord.Embed(title=t('music_removequeue_title'), description=t('music_removequeue_error_invalid', count=len(queue)), color=discord.Color.red())
            embed.set_author(name=t('help_requested_by', user=interaction.user.name), icon_url=interaction.user.avatar)
            embed.set_footer(text=get_current_version(self.client))
            return await interaction.response.send_message(embed=embed, ephemeral=True)

        removed = queue.pop(position - 1)
        embed = discord.Embed(title=t('music_removequeue_title'), description=t('music_removequeue_desc', title=removed['title']), color=discord.Color.green())
        embed.set_author(name=t('help_requested_by', user=interaction.user.name), icon_url=interaction.user.avatar)
        embed.set_footer(text=get_current_version(self.client))
        await interaction.response.send_message(embed=embed)
    ...
    async def mseek(self, interaction: discord.Interaction, minutes: int = 0, seconds: int = 0):
        ...
        if track and track.get('duration') and total_seconds > track['duration']:
            embed = discord.Embed(title=t('yt_error_title'), description=t('music_seek_error_duration', duration=self.audio_service.format_time(track['duration'])), color=discord.Color.red())
            return await interaction.followup.send(embed=embed, ephemeral=True)

        await self.audio_service.seek(interaction.guild, total_seconds, lambda e: self.check_queue(interaction))
        
        embed = discord.Embed(title=t('music_seek_title'), description=t('music_seek_desc', time=self.audio_service.format_time(total_seconds)), color=discord.Color.green())
        embed.add_field(name=t('yt_progress'), value=self.audio_service.create_progress_bar(interaction.guild.id), inline=False)
        embed.set_author(name=t('help_requested_by', user=interaction.user.name), icon_url=interaction.user.avatar)
        embed.set_footer(text=get_current_version(self.client))
        await interaction.followup.send(embed=embed)
    ...
    async def mprevious(self, interaction: discord.Interaction):
        ...
        if previous:
            ...
            embed = discord.Embed(title=t('music_previous_title'), description=t('music_previous_desc', title=previous['title']), color=discord.Color.green())
            embed.set_author(name=t('help_requested_by', user=interaction.user.name), icon_url=interaction.user.avatar)
            embed.set_footer(text=get_current_version(self.client))
            await interaction.followup.send(embed=embed)
        else:
            embed = discord.Embed(title=t('music_previous_title'), description=t('music_previous_error'), color=discord.Color.red())
            embed.set_author(name=t('help_requested_by', user=interaction.user.name), icon_url=interaction.user.avatar)
            embed.set_footer(text=get_current_version(self.client))
            await interaction.followup.send(embed=embed, ephemeral=True)
    ...
    async def mvolume(self, interaction: discord.Interaction, volume: int):
        if volume < 0 or volume > 200:
            embed = discord.Embed(title=t('music_volume_error_title'), description=t('music_volume_error_range'), color=discord.Color.red())
            embed.set_author(name=t('help_requested_by', user=interaction.user.name), icon_url=interaction.user.avatar)
            embed.set_footer(text=get_current_version(self.client))
            return await interaction.response.send_message(embed=embed, ephemeral=True)
        ...
        embed = discord.Embed(title=t('music_volume_title'), description=t('music_volume_desc', volume=volume), color=discord.Color.green())
        embed.set_author(name=t('help_requested_by', user=interaction.user.name), icon_url=interaction.user.avatar)
        embed.set_footer(text=get_current_version(self.client))
        await interaction.response.send_message(embed=embed)
